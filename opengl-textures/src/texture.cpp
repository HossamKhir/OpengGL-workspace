...
const GLchar* pglcTextureVertex120 = R"END(
	 #version 120
...
const GLchar* pglcTextureRaster120 = R"END(
	 #version 120
...
texture(void)
{
	...
	glShaderSource(gluVertexShader, 1, &pglcTextureVertex120, 0);
	...
	glShaderSource(gluFragmentShader, 1, &pglcTextureRaster120, 0);
	...
	//============================================================================
	// compilers usually optimise the memory, we do not need that for some vague reason now
	...
	struct pixel {
		...
		GLubyte glubGreen;
		GLubyte glubBlue;
		// extra align bytes added as required
	};
	...
	gluUniformMatrix4fv(gluiAttribMatrix, 1/*it is one unit of matrix*/, GL_FALSE, glfMatrix);
